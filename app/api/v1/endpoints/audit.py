...
async def get_audit_logs(
    ...
    entity_type: Optional[str] = Query(
        None, description="Filter by entity type (loan, payment, user)"
    ),
...
async def get_user_activity(
    ...
    query = (
        select(AuditLog)
        .where(and_(AuditLog.actor_id == user_id, AuditLog.timestamp >= cutoff_time))
        .order_by(desc(AuditLog.timestamp))
        .limit(limit)
    )
...
async def get_audit_statistics(
    ...
    user_query = (
        ...
        .where(AuditLog.timestamp >= cutoff_time)
        .group_by(AuditLog.actor_name, AuditLog.actor_type)
        .order_by(desc("count"))
    )
    ...
    return {
        ...
        "by_action": [{"action": row.action, "count": row.count} for row in actions],
        "by_entity_type": [
