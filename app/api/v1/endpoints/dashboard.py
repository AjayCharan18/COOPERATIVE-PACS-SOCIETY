...
async def get_dashboard_overview(
    ...
    farmers_query = select(func.count(UserModel.id)).where(UserModel.role == UserRole.FARMER)
    if current_user.role == UserRole.EMPLOYEE:
        farmers_query = farmers_query.where(UserModel.branch_id == current_user.branch_id)
    ...
    recent_query = select(func.count(Loan.id)).where(
...
async def get_monthly_statistics(
    ...
    ).group_by(func.date_trunc('month', Loan.disbursement_date)).order_by(
        func.date_trunc('month', Loan.disbursement_date).desc()
    )
...
async def get_farmer_statistics(
    ...
    top_farmers_query = select(
        ...
        func.sum(Loan.total_outstanding).label('total_outstanding')
    ).group_by(Loan.farmer_id).order_by(
    ...
    active_farmers_query = select(
        func.count(func.distinct(Loan.farmer_id))
    ).where(Loan.status == LoanStatus.ACTIVE)
...
async def get_performance_metrics(
    ...
    portfolio_query = select(
        func.count(Loan.id).label('total_loans'),
        func.sum(case((Loan.status == LoanStatus.ACTIVE, 1), else_=0)).label('active_loans'),
        func.sum(case((Loan.status == LoanStatus.DEFAULTED, 1), else_=0)).label('defaulted_loans'),
        func.sum(case((Loan.status == LoanStatus.CLOSED, 1), else_=0)).label('closed_loans'),
...
async def get_admin_system_overview(
    ...
    users_query = select(
        ...
        func.count(case((UserModel.role == UserRole.EMPLOYEE, 1))).label('total_employees'),
    ...
    loans_query = select(
        func.count(Loan.id).label('total_loans'),
        func.count(case((Loan.status == LoanStatus.PENDING_APPROVAL, 1))).label('pending'),
        ...
        func.coalesce(func.sum(case((Loan.total_outstanding > 0, Loan.total_outstanding))), 0).label('portfolio_at_risk')
    )
    ...
    branch_performance_query = select(
        ...
        UserModel, and_(
            UserModel.branch_id == Branch.id,
            UserModel.role == UserRole.FARMER
        )
    ).group_by(Branch.id, Branch.name, Branch.code).order_by(
        func.coalesce(func.sum(Loan.principal_amount), 0).desc()
    )
    ...
    return {
        ...
        "recent_activity": {
            ...
            "last_loan_date": recent_loans[0].created_at.isoformat() if recent_loans else None
        },
...
async def get_branch_analytics(
    ...
    for branch in branches:
        ...
        npa_amount = float(npa_stats.npa_amount or 0)
        npa_percentage = (npa_amount / total_outstanding * 100) if total_outstanding > 0 else 0
        ...
        loans_per_employee = loan_stats.total_loans / employee_count if employee_count > 0 else 0
        farmers_per_employee = farmer_count / employee_count if employee_count > 0 else 0
        avg_loan_size = total_outstanding / loan_stats.active if loan_stats.active else 0
    ...
    total_outstanding = sum(b["loan_statistics"]["outstanding_balance"] for b in branch_analytics)
    total_collections = sum(b["collection_statistics"]["this_month_amount"] for b in branch_analytics)
    total_npa = sum(b["npa_statistics"]["npa_amount"] for b in branch_analytics)
    
    # Rank branches by performance
    sorted_by_outstanding = sorted(branch_analytics, key=lambda x: x["loan_statistics"]["outstanding_balance"], reverse=True)
    sorted_by_collections = sorted(branch_analytics, key=lambda x: x["collection_statistics"]["this_month_amount"], reverse=True)
    sorted_by_npa = sorted(branch_analytics, key=lambda x: x["npa_statistics"]["npa_percentage"])
    ...
    return {
        ...
        "system_totals": {
            ...
            "system_npa_percentage": round((total_npa / total_outstanding * 100) if total_outstanding > 0 else 0, 2)
        },
        "rankings": {
            "top_by_outstanding": [
                {
