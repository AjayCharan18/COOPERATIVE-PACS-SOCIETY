...
from app.schemas.loan import (
    LoanCreate, Loan as LoanSchema, LoanUpdate, LoanApproval,
    LoanReschedule, LoanDetail, LoanSummary, LoanWithSchedule
)
...
async def get_loan_types(
    ...
    for lt in loan_types:
        ...
        loan_type_value = lt.loan_type.value if hasattr(lt.loan_type, 'value') else str(lt.loan_type)
        ...
        response_data.append({
            ...
            "interest_calculation_type": lt.interest_calculation_type.value if hasattr(lt.interest_calculation_type, 'value') else str(lt.interest_calculation_type),
...
async def get_risk_assessment(
    ...
    result = await db.execute(
        select(Loan)
        .options(selectinload(Loan.farmer))
        .where(Loan.id == loan_id)
    )
    loan = result.scalar_one_or_none()
