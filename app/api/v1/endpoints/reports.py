...
async def export_loans_csv(
    ...
    if from_date:
        filters.append(Loan.created_at >= datetime.combine(from_date, datetime.min.time()))
    if to_date:
        filters.append(Loan.created_at <= datetime.combine(to_date, datetime.max.time()))
...
async def export_emi_schedule_csv(
    ...
    filename = f"emi_schedule_{loan.loan_number}_{datetime.now().strftime('%Y%m%d')}.csv"
...
async def get_loan_summary_report(
    ...
    total_disbursed = sum(loan.principal_amount for loan in loans if loan.disbursement_date)
    total_outstanding = sum(loan.total_outstanding for loan in loans if loan.status == LoanStatus.ACTIVE)
...
async def get_monthly_report(
    ...
    try:
        ...
        print(f"DEBUG: Generating report for {first_day} to {last_day}", file=sys.stderr)
        sys.stderr.flush()
        ...
        total_disbursed_amount = sum(float(loan.principal_amount) for loan in disbursed_loans)
        ...
        emi_query = select(EMISchedule).where(
            EMISchedule.due_date >= first_day,
            EMISchedule.due_date <= last_day
        )
        ...
        amount_collected = sum(float(emi.paid_amount or 0) for emi in emis_due if emi.is_paid)
        
        collection_rate = (emis_paid / total_emis_due * 100) if total_emis_due > 0 else 0
        collection_efficiency = (amount_collected / total_amount_due * 100) if total_amount_due > 0 else 0
