...
async def get_calculator_loans(
    ...
    for loan in loans:
        ...
        outstanding = (
            loan.total_outstanding if loan.total_outstanding else loan.principal_amount
        )
...
async def calculate_interest_for_days(
    ...
    try:
        ...
        query = select(LoanTypeConfig).where(
            LoanTypeConfig.loan_type == request.loan_type
        )
        result = await db.execute(query)
        ...
        if not loan_config:
            raise HTTPException(
                status_code=404, detail=f"Loan type {request.loan_type} not found"
            )
        ...
        loan_type_value = (
            request.loan_type
            if isinstance(request.loan_type, str)
            else request.loan_type.value
        )
        ...
        if request.days <= 365:
            ...
            rate_breakdown.append(
                {"days": 365, "rate": base_rate, "interest": round(interest_year_1, 2)}
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error calculating interest: {str(e)}"
        )
...
async def calculate_interest_projections(
    ...
    try:
        result = await calculator.calculate_interest_today_tomorrow_future(
            request.loan_id
        )
        return result
    except Exception as e:
...
async def get_smart_recommendations(
    ...
    try:
        result = await calculator.get_smart_recommendations(
            loan_id=request.loan_id, farmer_monthly_income=request.farmer_monthly_income
        )
        return result
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def ai_explain_calculation(
    ...
    try:
        explanation = await calculator.explain_with_ai(
            calculation_data=request.calculation_data, language=request.language
        )
        return {
...
async def suggest_repayment_plan(
    ...
    try:
        suggestion = await calculator.suggest_repayment_plan(
            loan_id=request.loan_id, farmer_income=request.farmer_income
        )
        return suggestion
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def run_batch_calculation(
    ...
    try:
        result = await accrual_service.run_batch_calculation(
            calculation_type=calculation_type, as_of_date=as_of_date
        )
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
            ...
            "jobs": [
                {
                    ...
                    "total_accrual": (
                        float(job.total_accrual_amount)
                        if job.total_accrual_amount
                        else 0
                    ),
