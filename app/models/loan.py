...
class Loan(Base, TimestampMixin):
    ...
    loan_type_config_id = Column(
        Integer, ForeignKey("loan_type_configs.id"), nullable=True
    )
    ...
    # Calculation method
    interest_calculation_type = Column(
        SQLEnum(InterestCalculationType), default=InterestCalculationType.PRORATA_DAILY
    )
    ...
    ledger_entries = relationship(
        "LoanLedger", back_populates="loan", cascade="all, delete-orphan"
    )
    emi_schedule = relationship(
        "EMISchedule", back_populates="loan", cascade="all, delete-orphan"
    )
