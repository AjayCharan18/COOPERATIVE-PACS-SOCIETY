...
class BranchService:
    ...
    async def get_branch_statistics(db: AsyncSession, branch_id: int) -> Dict:
        ...
        active_loans = await db.scalar(
            select(func.count(Loan.id)).where(
                Loan.branch_id == branch_id, Loan.status == LoanStatus.ACTIVE
            )
        )

        # Total disbursed amount
        total_disbursed_result = await db.execute(
            select(func.sum(Loan.principal_amount)).where(
                Loan.branch_id == branch_id,
                Loan.status.in_([LoanStatus.ACTIVE, LoanStatus.CLOSED]),
            )
        )
        ...
        outstanding_result = await db.execute(
            select(func.sum(Loan.outstanding_principal)).where(
                Loan.branch_id == branch_id, Loan.status == LoanStatus.ACTIVE
            )
        )
        ...
        collected_result = await db.execute(
            select(func.sum(Loan.total_paid)).where(Loan.branch_id == branch_id)
        )
        total_collected = collected_result.scalar() or 0
        collection_rate = (
            (total_collected / total_disbursed * 100) if total_disbursed > 0 else 0
        )
    ...
    async def get_branch_monthly_trend(
        db: AsyncSession, branch_id: int, months: int = 6
    ) -> List[Dict]:
