...
class DailyAccrualService:
    ...
    async def _accrue_interest_for_loan(
        self, loan: Loan, accrual_date: date
    ) -> Decimal:
        """
    ...
    async def post_payment_to_ledger(
        self, loan_id: int, payment_amount: Decimal, payment_date: date, payment_id: int
    ):
        """
    ...
    async def run_batch_calculation(
        self, calculation_type: str, as_of_date: date = None
    ) -> Dict:
        """
        ...
        for loan in active_loans:
            try:
                ...
                results.append({"loan_id": loan.id, "result": result})
    ...
    async def _create_or_update_job(
        self, accrual_date: date, status: str
    ) -> AccrualJob:
    ...
    async def _get_ledger_entry(
        self, loan_id: int, transaction_date: date, transaction_type: str
    ) -> LoanLedger:
    ...
    async def _get_latest_balance(self, loan_id: int, before_date: date) -> Decimal:
        ...
        loan_result = await self.db.execute(select(Loan).where(Loan.id == loan_id))
