...
class DocumentService:
    ...
    def validate_file(filename: str, file_size: int) -> tuple[bool, str]:
        ...
        if ext not in DocumentService.ALLOWED_EXTENSIONS:
            return (
                False,
                f"File type {ext} not allowed. Allowed types: {', '.join(DocumentService.ALLOWED_EXTENSIONS)}",
            )
        ...
        if file_size > DocumentService.MAX_FILE_SIZE:
            return (
                False,
                f"File size exceeds maximum limit of {DocumentService.MAX_FILE_SIZE / (1024*1024)}MB",
            )
