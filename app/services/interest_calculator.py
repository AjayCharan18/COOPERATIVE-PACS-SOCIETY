...
class InterestCalculator:
    ...
    def calculate_prorata_interest(
        principal: float, annual_rate: float, from_date: date, to_date: date
    ) -> float:
        """
    ...
    def generate_emi_schedule(
        principal: float, annual_rate: float, tenure_months: int, start_date: date
    ) -> list[dict]:
        """
    ...
    def calculate_penal_interest(
        outstanding_amount: float, penal_rate: float, overdue_days: int
    ) -> float:
