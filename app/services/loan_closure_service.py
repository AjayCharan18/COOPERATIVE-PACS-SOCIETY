...
class LoanClosureService:
    ...
    async def calculate_closure_amount(db: AsyncSession, loan_id: int) -> Dict:
        ...
        # Calculate totals
        total_emi_outstanding = sum(emi.emi_amount - emi.paid_amount for emi in unpaid_emis)
    ...
    async def get_closed_loans_summary(db: AsyncSession, branch_id: int = None) -> Dict:
        ...
        return {
            ...
            "average_loan_amount": round(total_disbursed / len(closed_loans), 2) if closed_loans else 0,
            "closure_rate": round((len(closed_loans) / await db.scalar(select(Loan).count())) * 100, 2) if await db.scalar(select(Loan).count()) > 0 else 0
        }
