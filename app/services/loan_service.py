...
class LoanService:
    ...
    async def create_loan(
        db: AsyncSession, loan_data: LoanCreate, created_by: User
    ) -> Loan:
        ...
        result = await db.execute(
            select(LoanTypeConfig).where(
                LoanTypeConfig.loan_type == loan_data.loan_type
            )
        )
        ...
        loan_number = (
            f"LOAN{datetime.now().strftime('%Y%m%d')}{str(uuid.uuid4())[:8].upper()}"
        )

        # Calculate maturity date
        maturity_date = loan_data.sanction_date + timedelta(
            days=30 * loan_data.tenure_months
        )
    ...
    async def approve_loan(
        db: AsyncSession, approval_data: LoanApproval, approved_by: User
    ) -> Loan:
        ...
        if loan.status != LoanStatus.PENDING_APPROVAL:
            raise ValueError(
                f"Loan is not in pending approval status. Current status: {loan.status}"
            )
        ...
        if approval_data.approved:
            ...
            if approval_data.disbursement_date:
                ...
                if loan.emi_amount and not loan.first_emi_date:
                    loan.first_emi_date = approval_data.disbursement_date + timedelta(
                        days=30
                    )
    ...
    async def reschedule_loan(
        db: AsyncSession, reschedule_data: LoanReschedule, user: User
    ) -> Loan:
        """Reschedule an existing loan"""

        result = await db.execute(
            select(Loan).where(Loan.id == reschedule_data.loan_id)
        )
        ...
        new_loan = Loan(
            ...
            + timedelta(days=30 * reschedule_data.new_tenure_months),
    ...
    async def get_loan_summary(db: AsyncSession, filters: dict = None) -> dict:
        ...
        total_outstanding = sum(
            l.total_outstanding for l in loans if l.status == LoanStatus.ACTIVE
        )
        ...
        overdue_loans = [
            l
            for l in loans
            if l.status == LoanStatus.ACTIVE and l.maturity_date < date.today()
        ]
        overdue_amount = sum(l.total_outstanding for l in overdue_loans)

        # NPA loans (overdue > 90 days)
        npa_loans = [
            l for l in overdue_loans if (date.today() - l.maturity_date).days > 90
        ]
