...
class MLService:
    ...
    async def predict_default_risk(
        ...
        result = await db.execute(select(Loan).where(Loan.farmer_id == farmer.id))
        previous_loans = result.scalars().all()

        # Calculate features
        features = MLService._extract_features(
            farmer, previous_loans, loan_amount, tenure_months
        )
    ...
    def _extract_features(
        farmer: User, previous_loans: list, loan_amount: float, tenure_months: int
    ) -> Dict:
        ...
        if previous_loans:
            ...
            defaulted = sum(
                1 for l in previous_loans if l.status == LoanStatus.DEFAULTED
            )
            closed_on_time = sum(
                1 for l in previous_loans if l.status == LoanStatus.CLOSED
            )

            payment_history_score = (
                (closed_on_time / total_loans) * 100 if total_loans > 0 else 50
            )
        ...
        features["loan_amount_normalized"] = min(
            loan_amount / 1000000, 1.0
        )  # Cap at 10L
    ...
    async def identify_high_risk_loans(
        db: AsyncSession, threshold: float = 0.6
    ) -> list:
        """
        ...
        for loan in active_loans:
            ...
            payment_ratio = (
                ...
                if loan.total_amount_payable > 0
                else 0
            )
            ...
            risk_factors = {
                ...
                "high_outstanding": loan.total_outstanding
                > loan.principal_amount * 0.8,
            }
