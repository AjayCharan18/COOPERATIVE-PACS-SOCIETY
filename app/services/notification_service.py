...
from app.models.notification import Notification, NotificationTemplate, NotificationType, NotificationPurpose, NotificationStatus
...
class NotificationService:
    ...
    async def send_notification(
        ...
        message_template = getattr(template, f"template_{language}", template.template_english)
        title_template = getattr(template, f"subject_{language}", template.subject_english) or "Notification"
    ...
    async def _send_email(email: str, subject: str, body: str):
        ...
        msg['From'] = settings.EMAIL_FROM if hasattr(settings, 'EMAIL_FROM') else settings.SMTP_USER
        msg['To'] = email
        msg['Subject'] = subject
    ...
    async def send_emi_reminder(
        ...
        context = {
            ...
            "due_date": loan.first_emi_date.strftime("%d-%b-%Y") if loan.first_emi_date else "N/A",
            "days": days_before
        }
