...
class PaymentService:
    ...
    async def process_payment(
        ...
        allocation = await PaymentService._allocate_payment(
            loan, payment_data.amount, payment_data.payment_date
        )
        ...
        loan.total_outstanding = (
            loan.outstanding_principal + loan.outstanding_interest + loan.penal_interest
        )
    ...
    @staticmethod
    async def _allocate_payment(loan: Loan, amount: float, payment_date: date) -> dict:
        """
    ...
    @staticmethod
    async def _update_emi_schedule(db: AsyncSession, loan: Loan, payment: Payment):
        """Update EMI schedule after payment"""
        # Find unpaid EMIs
        result = await db.execute(
            select(EMISchedule)
            .where(EMISchedule.loan_id == loan.id, EMISchedule.is_paid == False)
    ...
    @staticmethod
    async def get_loan_ledger(db: AsyncSession, loan_id: int) -> list[LoanLedger]:
