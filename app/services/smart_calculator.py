...
class SmartCalculator:
    ...
    async def calculate_pro_rata_interest(
        ...
        if crosses_one_year:
            ...
            rate_after = await self._get_rate_for_period(loan, one_year_date + timedelta(days=1))
            
            interest_before = self._calculate_daily_interest(principal, rate_before, days_before)
            interest_after = self._calculate_daily_interest(principal, rate_after, days_after)
            ...
            interest = self._calculate_daily_interest(principal, annual_rate, total_days)
    ...
    async def calculate_interest_today_tomorrow_future(self, loan_id: int) -> Dict:
        ...
        interest_10_days = await self._calculate_interest_till_date(loan_id, after_10_days)
        interest_next_month = await self._calculate_interest_till_date(loan_id, next_month)
    ...
    async def calculate_overdue_with_penalty(
        ...
        overdue_interest = self._calculate_daily_interest(overdue_amount, overdue_rate, overdue_days)
    ...
    async def generate_emi_amortization_table(self, loan_id: int) -> Dict:
        ...
        if not loan.emi_amount or loan.emi_amount == 0:
            return {
                "error": "This is not an EMI-based loan",
                "loan_id": loan_id
            }
        
        # Ensure all values are Decimal
        principal = loan.principal_amount if isinstance(loan.principal_amount, Decimal) else Decimal(str(loan.principal_amount))
        interest_rate = loan.interest_rate if isinstance(loan.interest_rate, Decimal) else Decimal(str(loan.interest_rate))
        emi = loan.emi_amount if isinstance(loan.emi_amount, Decimal) else Decimal(str(loan.emi_amount))
    ...
    async def get_loan_snapshot_on_date(
        self, 
        loan_id: int, 
        as_of_date: date
    ) -> Dict:
    ...
    async def simulate_early_payment(
        ...
        """
        loan = await self._get_loan(loan_id)
        current_outstanding = await self._get_outstanding_principal(loan_id, payment_date)
        ...
        remaining_months = loan.tenure_months - self._get_months_elapsed(loan, payment_date)
        new_maturity = payment_date + timedelta(days=remaining_months * 30)
        
        interest_saved = old_total_interest - (new_emi * remaining_months - new_outstanding)
        ...
        return {
            ...
            "current_maturity": loan.maturity_date.isoformat() if loan.maturity_date else None,
    ...
    async def simulate_prepayment(
        ...
        """
        loan = await self._get_loan(loan_id)
        current_outstanding = await self._get_outstanding_principal(loan_id, prepayment_date)
        
        new_outstanding = current_outstanding - prepayment_amount
        remaining_months = loan.tenure_months - self._get_months_elapsed(loan, prepayment_date)
        ...
        return {
            ...
            "explanation": self._explain_prepayment(
                prepayment_amount, reduce_emi, loan.emi_amount, 
                new_emi, remaining_months, new_tenure
            )
        }
    ...
    async def apply_rate_switching(self, loan_id: int, as_of_date: date) -> Dict:
        ...
        if new_rate != current_rate:
            await self._log_rate_change(loan_id, current_rate, new_rate, as_of_date, rule_applied)
        ...
        return {
            ...
            "switch_date": switch_date.isoformat() if new_rate != current_rate else None,
            "explanation": (
                f"Rate switched from {current_rate}% to {new_rate}% "
                f"because loan passed 365 days on {switch_date.strftime('%d-%b-%Y')}"
            ) if new_rate != current_rate else "No rate change required"
        }
    ...
    async def explain_with_ai(
        ...
        try:
            ...
            return calculation_data.get('explanation', 'Calculation completed successfully')
    
    async def suggest_repayment_plan(
        self, 
        loan_id: int, 
        farmer_income: Optional[Decimal] = None
    ) -> Dict:
        """
    ...
    async def _get_outstanding_principal(self, loan_id: int, as_of_date: date) -> Decimal:
        ...
        total_paid = sum((Decimal(str(p.amount)) if not isinstance(p.amount, Decimal) else p.amount) for p in payments) if payments else Decimal('0')
    ...
    async def _get_rate_for_period(self, loan: Loan, period_date: date) -> Decimal:
        ...
        base_rate = loan.interest_rate if isinstance(loan.interest_rate, Decimal) else Decimal(str(loan.interest_rate))
        ...
        if days_since_disbursement <= 365:
            ...
            loan_type_value = loan.loan_type.value if hasattr(loan.loan_type, 'value') else str(loan.loan_type)
    ...
    def _calculate_daily_interest(
        ...
        daily_rate = annual_rate / Decimal(365 * 100)  # Convert annual % to daily decimal
    ...
    async def _calculate_emi(
        ...
        monthly_rate = annual_rate / Decimal(1200)  # Annual to monthly, percentage to decimal
    ...
    async def _calculate_tenure(
        ...
        import math
        tenure = math.log(emi / (emi - principal * monthly_rate)) / math.log(1 + float(monthly_rate))
    ...
    async def compare_loan_schemes(
        ...
        for loan_type_str in loan_types:
            try:
                ...
                result = await self.db.execute(
                    select(LoanTypeConfig).where(LoanTypeConfig.loan_type == loan_type_enum)
                )
                ...
                if monthly_rate_base > 0:
                    ...
                    emi_scenario1 = principal * monthly_rate_base * factor_base / (factor_base - 1)
                else:
                ...
                if tenure_months <= 12:
                    ...
                    blended_rate = (interest_rate * Decimal(months_year1) + rate_after_year * Decimal(months_year2)) / Decimal(tenure_months)
                ...
                comparisons.append({
                    ...
                    "rate_after_year": float(rate_after_year) if tenure_months > 12 else None,
                    "emi_amount": float(emi),
                    "total_interest": float(total_interest),
                ...
                "comparisons": [
                    {
                        ...
                        "within_year_interest": comp["early_closure_benefit"]["total_interest"],
                        "savings": comp["early_closure_benefit"]["savings"]
                    }
                    for comp in comparisons if comp.get("early_closure_benefit")
                ]
            }
    ...
    async def get_smart_recommendations(
        ...
        outstanding = loan.outstanding_principal if isinstance(loan.outstanding_principal, Decimal) else Decimal(str(loan.outstanding_principal or loan.principal_amount))
        interest_rate = loan.interest_rate if isinstance(loan.interest_rate, Decimal) else Decimal(str(loan.interest_rate))
        ...
        if outstanding > Decimal("10000"):
            savings_10k = (Decimal("10000") * interest_rate / Decimal("100")) * Decimal(str(loan.tenure_months)) / Decimal("12")
        ...
        if loan.emi_amount:
            emi_decimal = loan.emi_amount if isinstance(loan.emi_amount, Decimal) else Decimal(str(loan.emi_amount))
        ...
        if farmer_monthly_income:
            income = farmer_monthly_income if isinstance(farmer_monthly_income, Decimal) else Decimal(str(farmer_monthly_income))
            if loan.emi_amount:
                emi_decimal = loan.emi_amount if isinstance(loan.emi_amount, Decimal) else Decimal(str(loan.emi_amount))
        ...
        days_since_disbursement = (date.today() - loan.disbursement_date).days if loan.disbursement_date else 0
        loan_type_value = loan.loan_type.value if hasattr(loan.loan_type, 'value') else str(loan.loan_type)
        ...
        if loan_type_value == "sao" and days_since_disbursement > 300:
            recommendations.append({
                ...
                "priority": "high",
                "potential_savings": float((outstanding * Decimal("6.75") / Decimal("100")))
            })
        
        # Sort by priority and potential savings
        priority_order = {"high": 0, "medium": 1, "low": 2}
        recommendations.sort(key=lambda x: (priority_order.get(x["priority"], 3), -x["potential_savings"]))
