...
async def check_farmer():
    async with AsyncSessionLocal() as db:
        ...
        result = await db.execute(
            select(User).where(User.email == "adiajay8684@gmail.com")
        )
        ...
        if user:
            ...
            print(f"\n=== Loans Filtering ===")
            result = await db.execute(
                select(Loan).where(Loan.farmer_id == user.farmer_id)
            )
            loans_by_farmer_id = result.scalars().all()
            print(
                f"Loans where farmer_id == user.farmer_id ({user.farmer_id}): {len(loans_by_farmer_id)}"
            )
            ...
            for loan in loans_by_id:
                print(
                    f"  - {loan.loan_number}: farmer_id={loan.farmer_id}, status={loan.status.value}, disbursed={loan.disbursement_date}"
                )
