    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            if branch:
                ...
                print(f"  - Village: {branch.village if hasattr(branch, 'village') else 'N/A'}")
            
            # Get loan types
            loan_types = await db.scalars(select(LoanTypeConfig).where(LoanTypeConfig.is_active == True))
            print(f"\nActive Loan Types:")
