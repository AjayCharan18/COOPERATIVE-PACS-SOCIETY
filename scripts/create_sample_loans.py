...
async def create_sample_loans():
    async with AsyncSessionLocal() as db:
        try:
            ...
            if existing_loans > 0:
                print(
                    f"✅ Found {existing_loans} existing loans for farmer: {farmer.full_name}"
                )
                return
            ...
            print(
                f"📝 Creating sample loans for farmer: {farmer.full_name} (ID: {farmer.id})"
            )
            ...
            for idx, loan_data in enumerate(sample_loans, 1):
                ...
                disbursement_date = (
                    ...
                    if loan_data["status"] in ["active", "closed"]
                    else None
                )
                ...
                loan = Loan(
                    ...
                    outstanding_principal=(
                        float(principal) if loan_data["status"] == "active" else 0.0
                    ),
                    outstanding_interest=(
                        float(interest) if loan_data["status"] == "active" else 0.0
                    ),
                    total_outstanding=(
                        float(total_amount) if loan_data["status"] == "active" else 0.0
                    ),
                    status=loan_data["status"],
                    sanction_date=application_date.date(),
                    disbursement_date=(
                        disbursement_date.date() if disbursement_date else None
                    ),
                    maturity_date=(
                        application_date + timedelta(days=months * 30)
                    ).date(),
