...
async def import_loans_from_csv(csv_file_path: str):
    ...
    async with AsyncSessionLocal() as db:
        ...
        for idx, loan_data in enumerate(loans_data, 1):
            try:
                print(
                    f"[{idx}/{len(loans_data)}] Processing loan for {loan_data['farmer_email']}..."
                )
                ...
                if not farmer:
                    print(
                        f"   ❌ Farmer '{loan_data['farmer_email']}' not found - SKIPPED"
                    )
                    failed_count += 1
                    continue
                ...
                result = await db.execute(
                    select(LoanTypeConfig).where(
                        LoanTypeConfig.display_name
                        == loan_data["loan_type_name"].strip(),
                ...
                if not loan_type:
                    print(
                        f"   ❌ Loan type '{loan_data['loan_type_name']}' not found - SKIPPED"
                    )
                    failed_count += 1
                    continue
                ...
                disbursement_date = datetime.strptime(
                    loan_data["disbursement_date"].strip(), "%Y-%m-%d"
                ).date()
                ...
                loan_number = (
                    f"LN-{disbursement_date.strftime('%Y%m%d')}-{loan_count + 1:04d}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file", type=str, help="Path to CSV file containing loan data"
    )
    parser.add_argument(
        "--create-sample", action="store_true", help="Create a sample CSV template"
    )
    ...
    if args.create_sample:
        ...
        print("\n  3. Import loans:")
        print(
            "     python scripts/import_loans_from_csv.py --file sample_loans_import.csv"
        )
        print("\n" + "=" * 80 + "\n")
