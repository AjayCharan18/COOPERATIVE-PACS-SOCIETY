...
class APITester:
    ...
    async def test_authentication(self, client: httpx.AsyncClient):
        ...
        try:
            response = await client.post(
                ...
                data={"username": "farmer@dccb.com", "password": "Farmer@123"},
            )
    ...
    async def test_loan_types(self, client: httpx.AsyncClient):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_success(
                    f"Retrieved {len(loan_types)} loan type configurations"
                )

                for lt in loan_types:
                    self.log_info(
                        f"- {lt['loan_type']}: {lt['max_amount']} max, {lt['interest_rate']}% interest"
                    )
            else:
    ...
    async def test_create_loan(self, client: httpx.AsyncClient):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_error(
                    f"Loan creation failed: {response.status_code} - {response.text}"
                )
    ...
    async def test_get_loan_details(self, client: httpx.AsyncClient):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_info(
                    f"Total Interest: ₹{loan.get('total_interest_amount', 'N/A')}"
                )
            else:
    ...
    async def test_approve_loan(self, client: httpx.AsyncClient):
        ...
        try:
            response = await client.post(
                f"{BASE_URL}/auth/login",
                data={"username": "employee@dccb.com", "password": "Employee@123"},
            )
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 200:
                    ...
                    self.log_error(
                        f"Loan approval failed: {response.status_code} - {response.text}"
                    )
            else:
    ...
    async def test_emi_schedule(self, client: httpx.AsyncClient):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_success(
                    f"EMI schedule retrieved - {len(schedule)} installments"
                )

                if schedule:
                    first_emi = schedule[0]
                    self.log_info(
                        f"First EMI: ₹{first_emi.get('emi_amount', 'N/A')} on {first_emi.get('due_date', 'N/A')}"
                    )
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for entry in ledger[:3]:  # Show first 3 entries
                    self.log_info(
                        f"- {entry.get('transaction_type', 'N/A')}: ₹{entry.get('amount', 0)} on {entry.get('transaction_date', 'N/A')}"
                    )
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for notif in notifications[:3]:  # Show first 3
                    self.log_info(
                        f"- {notif.get('title', 'N/A')}: {notif.get('notification_type', 'N/A')}"
                    )
            else:
                self.log_error(
                    f"Notifications retrieval failed: {response.status_code}"
                )
    ...
    async def test_reports(self, client: httpx.AsyncClient):
        ...
        try:
            response = await client.post(
                f"{BASE_URL}/auth/login",
                data={"username": "admin@dccb.com", "password": "Admin@123"},
            )
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 200:
                    ...
                    self.log_info(
                        f"Total Disbursed: ₹{stats.get('total_disbursed', 0)}"
                    )
                else:
