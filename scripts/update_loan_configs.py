    ...
    async with AsyncSessionLocal() as db:
        ...
        for config in configs:
            if config.loan_type == LoanType.SAO:
                ...
                config.description = (
                    "Short-term crop loans - 1 year: 7%, Above 1 year: 8.5%"
                )
        ...
        result = await db.execute(
            select(LoanTypeConfig).where(
                LoanTypeConfig.loan_type == LoanType.RYTHU_NETHANY
            )
        )
        ...
        if not existing:
            ...
            existing.description = (
                "EMI installment over 10 years - 1 year: 12.50%, Above 1 year: 14.50%"
            )
